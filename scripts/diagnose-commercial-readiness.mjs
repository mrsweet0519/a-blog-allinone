...
const summarizeForConsole = (item = {}) => ({
  ...
  revisionEffectiveness: item.revisionEffectiveness,
  revisionGain: item.revisionGain,
  ...
  effectiveTargetCharCount: item.effectiveTargetCharCount,
  rawWriterCharCount: item.rawWriterCharCount,
  actualCharCount: item.actualCharCount,
  finalCharCount: item.finalCharCount,
  targetComplianceRatio: item.targetComplianceRatio,
  sectionBudgetTotal: item.sectionBudgetTotal,
  sectionActualTotal: item.sectionActualTotal,
  targetLengthFailureReason: item.targetLengthFailureReason,
  postProcessingReductionRatio: item.postProcessingReductionRatio,
...
export const runCommercialDiagnostics = async ({
  ...
  for (const [index, input] of inputs.entries()) {
    ...
    const summary = {
      ...
      initialQualityScore: Number(revisionDiagnostics.initialQualityScore || 0),
      revisionEffectiveness: revisionDiagnostics.revisionEffectiveness || "",
      revisionGain: Number(revisionDiagnostics.revisionGain || 0),
      revisionReason: revisionDiagnostics.revisionReason || "",
      qualityAttempts: Number(base.qualityAttempts || 0),
      attemptScores: revisionDiagnostics.attemptScores || [],
      attemptDiagnostics: revisionDiagnostics.attempts || [],
      ...
      effectiveTargetCharCount: base.effectiveTargetCharCount || base.requestedTargetCharCount || input.targetCharCount,
      rawWriterCharCount: base.rawWriterCharCount ?? 0,
      actualCharCount: base.actualCharCount ?? charCount(body),
      finalCharCount: base.finalCharCount ?? base.actualCharCount ?? charCount(body),
      targetComplianceRatio: base.targetComplianceRatio ?? 0,
      sectionBudgetTotal: base.sectionBudgetTotal ?? 0,
      sectionActualTotal: base.sectionActualTotal ?? 0,
      targetLengthFailureReason: base.targetLengthFailureReason || "",
      postProcessingReductionRatio: base.postProcessingReductionRatio ?? 0,
