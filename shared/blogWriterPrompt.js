...
const buildWriterBrief = (pipelineContext = {}, { targetCharCount = 2500 } = {}) => {
  ...
  return {
    ...
    forbiddenClaims,
    unknownFields,
    claimBoundaries: factMap.claimBoundaries || null,
    constraintFactIds: factMap.factGroups?.constraintFactIds || [],
    experienceGuard,
    ...
    claimBoundary: {
      ...
      forbiddenClaims,
      unknownFields,
      allowedClaimTypes: experienceGuard.allowedClaimTypes || factMap.claimBoundaries?.allowedClaimTypes || [],
      forbiddenClaimTypes: experienceGuard.forbiddenClaimTypes || factMap.claimBoundaries?.forbiddenClaimTypes || [],
      inferenceRestrictedFields: factMap.claimBoundaries?.inferenceRestrictedFields || []
    },
    lengthContract: {
      requestedTargetCharCount,
      effectiveTargetCharCount,
...
export const buildBlogWriterUserPrompt = ({ form = {}, analysis = analyzeBlogWritingInput(form), fallbackDraft = null } = {}) => {
  ...
  return [
    ...
    "writerBrief.experienceGuard.allowedClaimTypes에 없는 claim type은 쓰지 마세요. writerBrief.claimBoundary.forbiddenClaimTypes와 inferenceRestrictedFields는 본문에서 반드시 피하세요.",
    "특히 product/beauty/fashion/underwear/comparison 계열에서 이미지가 없고 실제 사용 경험 fact가 없으면 직접 사용 경험, 사용 기간, 사용 장소, 사용 후 변화, 가족·동행자, 효과, 만족도, 불편 체감, 가격 만족, 배송 경험, 친절도, 상담 경험, 재구매 의사를 만들지 마세요.",
    "writerBrief.experienceGuard.mustUseReferenceTone이 true이면 '써보니', '사용해보니', '며칠 써봤다', '책상에 놓아보니', '확실히 느꼈다', '재구매 의사'처럼 실제 체험을 전제하는 표현을 쓰지 마세요.",
    ...
    "informationSufficiency가 high 또는 medium이면 effectiveTargetCharCount의 85~110% 안에 들어오도록 작성하세요. 글자수를 늘릴 때 일반론이나 꾸며낸 경험을 쓰지 말고 factMap.userFacts의 각 coverage fact를 서로 다른 문단에서 제품 정보, 비교 조건, 선택 기준으로 확장하세요.",
    ...
    "factMap.constraintFacts와 writerBrief.constraintFactIds는 본문에 그대로 반영할 fact가 아니라 작성 경계입니다. '직접 사용 후기는 아님', '본문에는 만들지 말고' 같은 제약 문장을 블로그 본문에 옮겨 쓰지 마세요.",
    ...
    "최종 본문에는 입력 사실 기준, claim ledger, unsupported claim, fact map, fact 판단, 검증 결과, 내부 판단, 위 조건을 반영하면, 자동 평가 기준 같은 내부 평가 문구를 넣지 마세요.",
