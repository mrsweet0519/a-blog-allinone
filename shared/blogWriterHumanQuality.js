...
export const HUMAN_GUIDE_PATTERN =
  /글을\s*읽는\s*사람|글\s*안에서|본문에서|광고처럼|작성하면|제공된\s*정보|메모에|단정하지\s*않고|자연스럽게\s*정리되는\s*느낌|글의\s*흐름|글이\s*구체적으로\s*완성|확인\s*필요|안전합니다|안전해요|해당\s*표현|식사\s*후보|작성\s*가이드|최종\s*검수표|입력\s*사실\s*기준|claim\s*ledger|unsupported\s*claim|fact\s*map|fact\s*판단|검증\s*결과|내부\s*판단|위\s*조건을\s*반영하면|자동\s*평가\s*기준/u;
...
const PLACEHOLDER_PATTERN =
  /해당\s*(?:제품|서비스|업체|장소|메뉴|상품)|대표\s*메뉴(?![가-힣A-Za-z0-9]*(?:\s*사진|\s*메뉴|인|으로|가|는|를|을))|사용자\s*메모|제공된\s*정보|확인\s*필요로\s*남깁|입력\s*사실\s*기준|claim\s*ledger|unsupported\s*claim|fact\s*map|fact\s*판단|검증\s*결과|내부\s*판단|위\s*조건을\s*반영하면|자동\s*평가\s*기준/u;
